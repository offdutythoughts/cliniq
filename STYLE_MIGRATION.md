...
- [~] **Wholesale "convert all 8 components to utilities" — deliberately scoped down.** Rationale,
      consistent with §0 ("the win is consolidation + tokens, **not** utilities everywhere"):
      (1) the chrome's stateful (`.show`/`.open`/`.active`) and pseudo (`::-webkit-scrollbar`,
      `:empty::before`, `::-webkit-color-swatch`, `::placeholder`) selectors can't be utilities and
      are cleaner as the retained `@layer components` classes; (2) the components' inline styles
      already use `var()` tokens; (3) `AccountMenu` + `/login` are **Convex-gated** (render null
      without a deployment) so the visual guardrail can't verify changes there — `login`'s `#fff`
      is correct white-on-accent and `AccountMenu`'s subtle shadow is benign, so both were left.
      The full chrome→utilities conversion is available as a follow-up if desired (would let the
      `.topbar`/`.nav-item`/`.notes-*` rules be deleted, keeping only the un-utility-able pseudos).
