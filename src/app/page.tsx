  ...
  return (
    <div className="site-page">
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-[1180px] items-center gap-14 px-5 pt-16 pb-20 sm:px-8 lg:grid-cols-[1.02fr_0.98fr] lg:gap-16 lg:pt-24 lg:pb-28">
            <div>
              ...
              <p className="mt-4 text-[13px] text-[var(--v-slate)]">
                No card needed to start. Cancel whenever.
              </p>
            </div>
        ...
        {/* ── Pricing + CTA ──────────────────────────────────────────────── */}
        <section>
          <div className="mx-auto grid max-w-[1180px] items-center gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:py-24">
            <div>
              <h2 className="v-display text-[34px] sm:text-[42px]">Try it on your next tricky case.</h2>
              <p className="mt-5 max-w-lg text-[16px] leading-[1.7] text-[var(--v-slate)]">
                Every plan includes the whole library — flowcharts, diagnostic approaches, disease
                pages, protocols and Mix &amp; Match. The first {TRIAL_DAYS} days are free.
              </p>
            ...
            <dl className="grid gap-3 sm:grid-cols-2">
              {PLANS.map((p) => (
                <div key={p.id} className="v-card p-6">
                  <dt className="flex items-center gap-2 text-[14px] font-semibold text-[var(--v-ink)]">
                    {p.name}
                    {p.badge && (
                      <span className="rounded-full bg-[var(--v-mist)] px-2 py-0.5 text-[10px] font-semibold tracking-[.04em] text-[var(--v-navy-deep)] uppercase">
                        {p.badge}
                      </span>
                    )}
                  </dt>
                  <dd>
                    <p className="mt-3">
                      <span className="v-display text-[30px] text-[var(--v-ink)]">{p.price}</span>
                      <span className="ml-1.5 text-[13px] text-[var(--v-slate)]">{p.cadence}</span>
                    </p>
                    <p className="mt-1.5 text-[12.5px] text-[var(--v-slate)]">{p.note}</p>
                  </dd>
                </div>
              ))}
            </dl>
