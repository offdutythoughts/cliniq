...
export const abnormalPupilDx: DxApproach = {
  ...
  history: {
    ...
    blocks: [
      ...
      // Agent → the question that confirms it. The band label carries the pupil
      // (mydriasis violet, miosis blue throughout the sign — the neuro branch
      // uses the same pair), so no per-row pupil cell repeats it; anything the
      // band does NOT say — bilateral, paradoxical, ipsilateral, ↓ IOP — leads
      // the clue. The dog/cat opioid split sits on adjacent bands because that
      // is where it is misread.
      {
        kind: 'gridTable',
        cols: '1fr 1.35fr',
        dividers: true,
        ...
        rows: [
          ...
          ['Head trauma', '<strong>Ipsilateral</strong> — CN III compression or intracranial bleed — <strong>neurosurgical emergency</strong>'],
        ],
      },
    ...
    after: [
      {
        ...
        title: '⚠️ RED FLAGS',
        items: [
          `Head trauma + ipsilateral mydriasis = rising ICP / CN III herniation (emergency MRI/decompression)`,
          `Acute bilateral mydriasis + blindness = optic neuritis or SARDS (urgent workup)`,
          `Fixed mid-dilated unresponsive pupil = acute glaucoma (refer same day)`,
          `Anisocoria + altered mentation / CN deficits / hemiparesis = central neurological emergency`,
        ],
      },
      { kind: 'disclaimer' },
    ],
  ...
  exam: {
    ...
    blocks: [
      ...
      { kind: 'step', text: '👁️ STEP 4 — SLIT-LAMP / FOCAL LIGHT — PUPIL SHAPE + IRIS', noArrowAfter: true },
      {
        kind: 'patterns',
        rows: [
          ...
          { section: 'Active intraocular disease' },
          { cues: ['Posterior synechiae adherent to the lens'], dx: 'Chronic uveitis — 2° glaucoma risk if 360°', tone: 'danger' },
          { cues: ['Aqueous flare', 'miosis', '↓ IOP'], dx: 'Anterior uveitis', tone: 'danger' },
          { cues: ['Mid-fixed mydriasis', 'corneal oedema', '↑ IOP'], dx: 'Acute glaucoma', tone: 'danger', emphasis: true },
          { cues: ['Lens visible in the AC / aphakic crescent'], dx: 'Anterior lens luxation', tone: 'danger', emphasis: true },
          { section: 'Neurological' },
          { cues: ['Miosis', 'ptosis', 'enophthalmos', '3rd eyelid'], dx: "Horner's syndrome", tone: 'warning' },
        ],
      },
