// Seizure diagnostic approach — History, Exam, and Diagnostics tabs

const dxTabs = (active: 'history' | 'exam' | 'dx') => `
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;margin-bottom:14px;">
  <div class="dx-step${active==='history'?'':' alt'}" style="padding:5px 4px;font-size:9px;cursor:pointer;text-align:center;${active==='history'?'':'opacity:.5;'}" onclick="renderDxSeizuresHistory()">📋 History</div>
  <div class="dx-step${active==='exam'?'':' alt'}" style="padding:5px 4px;font-size:9px;cursor:pointer;text-align:center;${active==='exam'?'':'opacity:.5;'}" onclick="renderDxSeizuresExam()">🩺 Exam</div>
  <div class="dx-step${active==='dx'?'':' alt'}" style="padding:5px 4px;font-size:9px;cursor:pointer;text-align:center;${active==='dx'?'':'opacity:.5;'}" onclick="renderDxSeizuresDx()">🔬 Diagnostics</div>
</div>
`

// ── Diagnostic approach — HISTORY ─────────────────────────────────────────────
export const seizureDxHistoryHtml = `
${dxTabs('history')}

<div class="dx-wrap">

  <div class="dx-step">📋 CHARACTERISE THE SEIZURE EVENT</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
    <strong>Ask owners to video future episodes.</strong> The description is the most important diagnostic tool — ask open questions, then clarify each phase:<br><br>
  ...
  <div class="dx-step alt">⏱ DURATION, FREQUENCY AND PROGRESSION</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
    <strong>Duration of each seizure:</strong> &lt;5 min (typical) or &gt;5 min (<strong>status epilepticus</strong> — treat immediately)<br>
  ...
  <div class="dx-step alt">💊 DRUG / TOXIN / MEDICATION HISTORY</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
  ...
  <div class="dx-step alt">🐾 SIGNALMENT AND BREED PREDISPOSITIONS</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 12px;font-size:9.5px;">
      <div>
...
export const seizureDxExamHtml = `
${dxTabs('exam')}

<div class="dx-wrap">

  <div class="dx-step">🚨 EMERGENCY ASSESSMENT FIRST</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
    <strong>Is the patient actively seizing or immediately post-ictal?</strong><br>
  ...
  <div class="dx-step alt">🧠 POSTICTAL vs INTERICTAL</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-row c2">
    <div class="dx-test" style="font-size:9px;">
      <strong style="color:#FCD34D;">Postictal</strong><br>
    ...
    <div class="dx-test" style="font-size:9px;">
      <strong style="color:#6EE7B7;">Interictal</strong><br>
  ...
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
    <strong style="color:#93C5FD;">Mentation:</strong> Alert (normal) / Obtunded (dull, slow responses) / Stuporous (minimal response to strong stimuli) / Comatose (no response). Head pressing suggests increased ICP. Compulsive circling (ipsilateral to lesion).<br><br>
  ...
  <div class="dx-step alt">🔍 PHYSICAL EXAMINATION</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-check">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 12px;font-size:9.5px;">
      <div>
...
<div style="margin-top:12px;padding:10px 14px;background:rgba(37,99,235,0.08);border:1px solid rgba(37,99,235,0.25);border-radius:12px;">
...
export const seizureDxDxHtml = `
${dxTabs('dx')}

<div class="dx-wrap">

  <div class="dx-step">⚡ IMMEDIATE IN-HOUSE (acute / post-SE)</div>
  <div class="dx-arrow">↓</div>
  <div class="dx-row c3">
    <div class="dx-test" style="font-size:9px;"><strong>Blood glucose</strong><br>Point-of-care<br>&lt;3.5 mmol/L → dextrose IV<br>&lt;2.5 mmol/L → urgent<br>→ insulinoma? PSS? Addison's?</div>
    <div class="dx-test" style="font-size:9px;"><strong>PCV / TS</strong><br>PCV &gt;60% → polycythaemia<br>PCV &lt;20% → cerebral hypoxia<br>TS ↑ → dehydration, protein</div>
    <div class="dx-test" style="font-size:9px;"><strong>Temperature</strong><br>T° &gt;41°C → active cooling<br>(wet towels, fan, cold IV fluids)<br>Stop cooling at 39.5°C</div>
  </div>
  <div class="dx-arrow">↓</div>
  ...
  <div class="dx-arrow">↓</div>
  <div class="dx-row c2">
    <div class="dx-test" style="font-size:9px;"><strong>MRI brain</strong> (1.5T or 3T)<br>FLAIR · T1 · T2 · T1+contrast<br>Gold standard for structural lesions<br>Mass? Infarct? Inflammation?<br>Periventricular → hydrocephalus</div>
    <div class="dx-test" style="font-size:9px;background:#0D7377;"><strong>CSF analysis</strong><br>Collect under GA after MRI<br>TNCC (normal &lt;5 cells/μL)<br>Protein (normal &lt;0.25 g/L)<br>Cytology + culture if indicated</div>
  </div>
  <div class="dx-arrow">↓</div>
...
<div style="margin-top:10px;padding:10px 14px;background:rgba(37,99,235,0.08);border:1px solid rgba(37,99,235,0.25);border-radius:12px;">
...
<div class="dx-alert" style="margin-top:8px;"><strong>⚠️ SE &gt;5min:</strong> Diazepam 0.5 mg/kg IV (×3) → Levetiracetam 20–60 mg/kg IV → Phenobarbital 2–5 mg/kg IV → Propofol CRI — see PROT-SEIZ</div>
<div class="disclaimer">For qualified veterinary professionals only.</div>
`
